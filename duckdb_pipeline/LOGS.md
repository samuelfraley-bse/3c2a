...
### Real bug found and fixed: mid-game possession resets silently fail to flip offense/defense when the team's own abbreviation isn't a text substring of its canonical name
- Follow-up to yesterday's kickoff-return-TD fix -- continuing to hand-verify Butte's game-by-game point totals surfaced a second, unrelated bug: `20251018_8uoz` (American River @ Butte) read 34-26, but the real score was 31-29. Not a scoring-formula bug this time -- an entire OT drive's `plays.offense`/`defense` were swapped.
- Root cause, traced to the raw source text: `"Start of 5th quarter, clock 15:00, ARC ball on BUTTE25."` -- `parse.py`'s `_resolve_possession_from_text` extracts the `ARC` token and checks whether it's a literal substring/truncation of either canonical team name (`"BAKERSFI"` matching `"Bakersfield"` this way already works and has a test). `ARC` is American River's real abbreviation but isn't textually related to `"American River"` at all, so the check silently returns `(None, None)` and the parser just keeps whatever offense/defense was already set from the previous (4th quarter) drive -- which happened to be Butte. Confirmed definitively by the ball carriers in the mislabeled drive (Nathan Crawford, Frank Arcuri) being American River's own real players, and by the field goal in that drive being kicked by Landon Richards, American River's real kicker, not Butte's.
- Scope, measured directly rather than estimated: scanned every stored raw play-by-play page across all 3 backfilled seasons for this exact `"TEAM ball on ..."` pattern -- **465 of 3,126** occurrences fail the substring check (163 in 2023-24, 178 in 2024-25, 124 in 2025-26). This is not an OT-only bug -- it hits any mid-game possession reset, OT just happened to be where it was first spotted. Cross-checked all 465 tokens against the project's own human-reviewed `field_position_crosswalk` table (the same one that already maps `ARC -> American River`, `DVC -> Diablo Valley`, etc. for field-position tokens) -- **all 465 already have a matching entry**, meaning this is fixable entirely from data the pipeline already has, with zero new manual review. Re-parsed the affected raw HTML with a patched resolver that falls back to the crosswalk and diffed against the currently stored `plays` rows: **120 distinct games, 190 individual play rows** get corrected offense/defense.
- Fixed in `parse.py`: added an optional `team_prefix_memory: dict[str, set[str]] | None` parameter threaded through `parse_pbp_html` into `_resolve_possession_from_text` (both call sites: the quarter-start branch and the generic per-play branch), used only as a fallback after the existing substring checks fail. Deliberately reuses `crosswalk.build_team_prefix_memory` (the same `{canonical_team: {known prefixes}}` map already built from `field_position_crosswalk` for the existing auto-refresh/preseed workflow) rather than inventing a second abbreviation-matching mechanism. Scoped to `_resolve_possession_from_text` only -- `_resolve_drive_start_possession` and the `<th>` drive-header path use the same `_team_matches` substring check but had no evidence of failing in the 465-occurrence scan, so left untouched rather than widening the change beyond what's proven necessary.
- Wired into both real call sites in `cli.py` (`main_plays` and `main_rebuild_plays_from_raw`): both now build `team_prefix_memory` via `build_team_prefix_memory(_load_crosswalk_rows(conn, season))` before parsing, so this benefits both a fresh scrape (reusing whatever crosswalk memory already exists from prior weeks) and a `rebuild_plays_from_raw` reparse of already-scraped seasons.
- Added `test_parse_pbp_html_quarter_start_ball_on_abbreviation_needs_crosswalk_memory` to `test_parse.py`, reproducing the exact `20251018_8uoz` OT text and asserting the possession stays wrong without `team_prefix_memory` and flips correctly with it. Full suite: 71 tests passing.

**Applied so far -- 2025-26 only:**
- Backed up first (`data/foothill.duckdb.bak-pre-possession-crosswalk-fix`), then:
  `uv run --active rebuild_plays_from_raw --season 2025-26 --db-path data/foothill.duckdb --source-plays-run-id 75e20b24-e705-463c-b966-59d32dd2d361`
  (that specific `--source-plays-run-id` matters -- it must be a run that actually has `raw_pbp_html` rows, i.e. an original scrape run, not a later reparse run like the previous "current best" `63fa3f95...`, which has none of its own and would fail with "No raw_pbp_html rows found").
...
### Real bug found and fixed: kickoff-return touchdowns credited to the kicking team instead of the return team
- Surfaced while the user was hand-verifying the dashboard's "Quick Hitters" PPG rank for Butte: a quick script summing `v_team_game_points_current` per game showed Butte 50-18 over Laney (`20250905_11sr`), but the real final score was Butte 38, Laney 30 -- a clean 12-point gap.
- Root cause: on a kickoff play, `plays.offense` stays the KICKING team for the whole play row even when the receiving team returns it for a touchdown -- unlike interceptions/fumbles, there's no separate parse-time flag distinguishing "kicking team's own play" from "return team scored." `v_play_context_current`'s `scoring_totals` CTE (`db.py`) was crediting raw `is_td=true` on a kickoff row straight to `offense_points` (the kicking team), when a kickoff can only ever be a touchdown via the return team scoring (barring the kicking team recovering its own forced fumble on the return, a separate, already-correctly-handled case).
- Confirmed by cross-referencing the very next play in both disputed cases: the PAT immediately following each mislabeled kickoff-TD was attempted by the *return* team's own kicker (e.g. Esteban Gonzalez, Laney's kicker, attempting Laney's extra point right after "his" team's kickoff-return score) -- proof the touchdown belonged to the return team, not the kicking team the raw row was tagged with.
- Scope: 110 kickoff-return-TD plays across all 3 backfilled seasons (26 in 2023-24, 44 in 2024-25, 40 in 2025-26) were affected -- every one previously double-counted against the true score (kicking team inflated by 6, return team shorted by 6, i.e. a 12-point swing per play on both `points_scored`/`points_allowed` and anything built on `offense_points`/`defense_points`: `v_team_season_points_ranked_current` (PPG/PPG-allowed and their ranks), `score_margin`/`score_margin_bucket`, and `v_drives_current`'s `is_scoring_drive`/`drive_points`.
- Fixed in `db.py`'s `scoring_points`/`scoring_totals` CTEs: added `is_kickoff_return_td` (`play_type='kickoff' AND is_td AND NOT is_conversion AND NOT is_fumble`), checked ahead of the generic `is_td` branch, flipping the 6 points to `defense_points` instead of `offense_points`. Deliberately excludes `is_fumble=true` kickoff rows -- the 4 real cases this season/others where the kicking team forces and recovers its own fumble on the return for a legitimate kicking-team touchdown are already handled correctly by the existing fumble-recovery crosswalk logic just above it in the same CASE, and must not be touched by this new branch.
- Added `test_kickoff_return_td_scores_to_return_team_not_kicking_team` to `test_db.py`, covering both the return-TD case (must flip to the return team) and the kicking-team's-own-fumble-recovery-TD case (must NOT flip) in the same fixture, asserting both teams end up 7-7 instead of the pre-fix 13-1. Full suite: 70 tests passing.
- Applied to the real database: backed up first (`data/foothill.duckdb.bak-pre-kickoff-return-td-fix`), then `refresh_duckdb_views` (pure view redefinition, no data rows touched). Verified directly against the real file: `20250905_11sr` now reads Butte 38 / Laney 30, matching the real final score exactly.
- Not yet done: the user is continuing to hand-check other Butte game point totals against real scores to look for further discrepancies -- this fix addresses the one confirmed class of bug found so far, not a guarantee every score is now correct.
