"""
Early Down Report — Offense & Defense
...
def compute_early_down(df: pd.DataFrame, group_col: str) -> pd.DataFrame:
    rows = []
    for team, g in df.groupby(group_col):
        ...
        rows.append({
            "team":          team,
            "yards_per_play":round(ed["yards_gained"].mean(), 2),
            "success_rt":    round(ed["success"].sum() / n * 100, 1) if n else 0,
            "explosive_pct": round(ed["explosive"].sum() / n * 100, 1) if n else 0,
            "rush_plays":    n_rush,
            ...
            "turnovers":     int((pass_att["pass_result"] == "int").sum() + ed["is_fumble"].sum()),
            "penalty_yards": int(ed["penalty_yards"].fillna(0).sum()),
        })
...
def main():
    ...
    off_stats  = compute_early_down(df, "offense")
    def_stats  = compute_early_down(df, "defense")
    off_ranked = add_ranks(off_stats, "offense")
    def_ranked = add_ranks(def_stats, "defense")

    off_ranked.to_csv(f"{OUT_DIR}/early_down_offense.csv", index=False)
    def_ranked.to_csv(f"{OUT_DIR}/early_down_defense.csv", index=False)

    report = build_report(off_ranked, def_ranked)
    report.to_csv(f"{OUT_DIR}/early_down_report.csv", index=False)
